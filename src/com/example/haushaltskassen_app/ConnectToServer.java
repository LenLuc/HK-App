...
public class ConnectToServer extends MyBaseActivity {
	...
	public void onclickDownload(View view) {
		...
		if(network){
			...
			String url = new String( "http://fathomless-fjord-2416.herokuapp.com/api/balances.json");
	...
	public static void downloadTable(String url)
	{
		...
		Log.d("httpget","bla");
		httpget.setHeader("Authorization","Token token=\"0910e8148e806403761ea524300a24dc\"");
		//  Log.d("setheader","bla");
		// Execute the request
	...
	public static void makeRequest(String path, List<Ausgabe> ausgaben) throws Exception 
	{
		...
		for (int i=0; i<holder.length();i++) {
			...
			httpost.setHeader("Authorization","Token token=\"0910e8148e806403761ea524300a24dc\"");
	...
	public static JSONArray getJsonObjectFromList(List<Ausgabe> ausgaben) throws JSONException{
		...
		if (ausgaben != null && ausgaben.size() > 0) {
			...
			for (Ausgabe selectedAusgaben : ausgaben) {
				...
				if(godera!=1){if(person==1){godera=2;} else{godera=1;};} //adapt to paid_for_user used by server
				//String personForServer="";
				if(person!=1){person=2;}//Piet=1, Lucia=2, adapt to personcode used by server
				json.put("cost", ""+selectedAusgaben.getBetrag());
				json.put("paid_for_user", ""+godera);
				json.put("user", ""+person);
				json.put("time_period", selectedAusgaben.getDate());
				array.put(json);
			}
